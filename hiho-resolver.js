	...
	for(var solution_id in this.solutions) {
		...
		if(sol.submitted_seconds <= this.frozen_seconds) {
            ...
            if(sol.verdict == 'AC') {
                ...
                var tmp = sol.verdict;
                if (old[0] == 'P') {
                    var num1 = parseFloat(old.substring(1, old.length));
                    var num2 = parseFloat(sol.verdict.substring(1, sol.verdict.length));
			...
			for(var j = 1; j <= this.problem_count; j++) {
				if(this.status(this.rank2[i].problem[j]) == "frozen") {
					...
					if(ver == 'AC') {
                        ...
                        if (ver2[0] == 'P') {
                            var num2 = parseFloat(ver2.substring(1, ver2.length));
                            num -= num2;
                        }
						tmp.score += num;
